                ...
                for alpha in cfg["alphas"]:
                    for T in cfg["walk_budgets"]:
                        for method in WALK_METHODS:
                            ...
                            execute_trials(args,cfg,experiment="walks",cohort=cohort,network=net,source_id=sid,sigma=sigma,alpha=alpha,method=method,T=T,K=K,B=B,residuals=method=="ppw")
    trials=read_trials(args.output); summary=summarize(args.output,args.stage2_results,{n.network_id:n for n in all_nets},trials)
