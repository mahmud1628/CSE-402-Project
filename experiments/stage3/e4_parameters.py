    ...
    for net in nets:
      for sid,sigma in source_specs(net,False):
       for alpha in cfg["alphas"]:
        for K in cfg["K_grid"]: execute_trials(args,cfg,experiment="parameters",cohort="secondary",network=net,source_id=sid,sigma=sigma,alpha=alpha,method="pw",T=T,K=K)
        # Fixed total propagation of 10 (actual K*B is recorded) rather than silently rounding a per-batch setting.
        for B in cfg["B_grid"]: execute_trials(args,cfg,experiment="parameters",cohort="secondary",network=net,source_id=sid,sigma=sigma,alpha=alpha,method="ppw",T=T,K=max(1,10//B),B=B,residuals=True)
    summary=summarize(args.output,args.stage2_results,{n.network_id:n for n in all_nets},read_trials(args.output)); d=summary[summary["experiment"].eq("parameters")]; plt=mpl(); fig,ax=plt.subplots(figsize=(6,4))
